...
for i in range(500):
    ...
    train_dict = {X: batch_X, Y_: batch_Y, lr: learning_rate, tst: False, pkeep: 0.75, pkeep_conv: 1.0}
    
    sess.run(train_step, feed_dict=train_dict)
    sess.run(update_ema, feed_dict={X: batch_X, Y_: batch_Y, tst: False, iter: i, pkeep: 1.0, pkeep_conv: 1.0})   ##train update_ema
    ...
    acc_and_loss = [(i+1), temp_train_loss, temp_train_acc, learning_rate]
    acc_and_loss = [np.round(x,3) for x in acc_and_loss]
    if((i+1) % 10 == 0):
        print('Generation # {}. Train Loss: {:.2f}. Train Acc : {:.2f}. learning_rate : {:.3f}'.format(*acc_and_loss))
...
for i in range(int(a/20)):
    ...
    test_dict = {X: eval_X, Y_: eval_Y, tst: True, pkeep: 1.0, pkeep_conv: 1.0}
  ...
  #  print(i, temp_test_acc)
    ...
    if(i % 10 == 0):
        print(i, ' Test Acc:', temp_test_acc)
